...
test("<!-- audio: --> comment on the next line overrides the alt text", () => {
  ...
  assert.deepEqual(texts(n), [
    "T",
    "Image: Long spoken description of the picture.",
    "Next.",
  ]);
});
...
test("caption that repeats the alt is read once", () => {
  ...
  assert.deepEqual(texts(n), [
    "T",
    "Figure 1: Hand pointing to the middle card. We continue.",
  ]);
});
...
test("caption that differs from the alt is read after it", () => {
  ...
  assert.deepEqual(texts(n), [
    "T",
    "Image: Photo of Hamilton next to a stack of listings.",
    "Hamilton in 1969.",
  ]);
});
...
test("empty alt without override throws with file and line", () => {
  assert.throws(
    ...
    (error) =>
      error instanceof NarrationError && /post\.md:8:/.test(error.message),
  );
});
...
test("fenced code blocks become one phrase, consecutive blocks collapse", () => {
  ...
  assert.deepEqual(texts(n), [
    "T",
    "Look:",
    "Here the written article shows a code example.",
    "Done inline.",
  ]);
});
...
test("MDX imports are dropped and CodePen embeds become a phrase", () => {
  ...
  assert.deepEqual(texts(n), [
    "T",
    "Text.",
    "Aquí el artículo incluye un ejemplo interactivo en CodePen.",
    "More.",
  ]);
});
