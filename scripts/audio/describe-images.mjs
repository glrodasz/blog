...
async function describe({ locale, imagePath, before, after, currentAlt }) {
  ...
  if (!endpoint || !key || !deployment) {
    throw new Error(
      "AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY and AZURE_OPENAI_DEPLOYMENT must be set",
    );
  }
  ...
  if (!response.ok)
    throw new Error(
      `Azure OpenAI ${response.status}: ${(await response.text()).slice(0, 300)}`,
    );
...
function context(raw, index) {
  ...
  const after =
    raw
      ...
      .filter(Boolean)[1] ?? "";
  return {
    before: before.trim().slice(-600),
    after: after.trim().slice(0, 600),
  };
}
...
for (const post of listPosts()) {
  ...
  for (const match of matches) {
    ...
    raw =
      raw.slice(0, start) + replacement + raw.slice(start + match[0].length);
...
console.log(
  `${updated} image(s) described${DRY_RUN ? " (dry run, nothing written)" : ""}.`,
);
