...
test("packChunks never splits a segment that fits and respects the limit", () => {
  ...
  for (const chunk of chunks)
    assert.ok(chunk.reduce((n, s) => n + s.text.length, 0) <= 100);
});
...
test("toSsml escapes text and wraps the voice", () => {
  const ssml = toSsml([seg("Título", "title"), seg('a < b & "c"')], {
    voice: "v",
    lang: "es-MX",
  });
  assert.match(
    ssml,
    /^<speak version="1.0" xmlns="http:\/\/www.w3.org\/2001\/10\/synthesis" xml:lang="es-MX"><voice name="v">/,
  );
...
test("synthesizeSsml retries on 429 and then succeeds", async () => {
  ...
  const audio = await synthesizeSsml("<speak/>", {
    key: "k",
    region: "r",
    fetchImpl,
  });
...
test("synthesizeSsml does not retry on 400", async () => {
  ...
  await assert.rejects(
    synthesizeSsml("<speak/>", { key: "k", region: "r", fetchImpl }),
    /400/,
  );
...
test("mp3Duration walks frames and ignores ID3 tags", () => {
  ...
  const id3 = Buffer.concat([
    Buffer.from("ID3\x04\x00\x00\x00\x00\x00\x0a", "latin1"),
    Buffer.alloc(10),
    twoSeconds,
  ]);
